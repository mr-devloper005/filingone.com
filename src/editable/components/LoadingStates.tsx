...
function PulseBlock({ className }: { className?: string }) {
  return <div className={cn('animate-pulse rounded-2xl bg-[linear-gradient(90deg,rgba(84,119,146,0.08),rgba(255,197,112,0.18),rgba(84,119,146,0.08))]', className)} />
}
  ...
  return (
    <div className={cn('mx-auto w-full max-w-[1440px] px-4 py-12 sm:px-6 lg:px-8', className)} aria-live="polite" aria-busy="true">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-[2rem] border border-[#1a32631f] bg-white/65 p-5 shadow-[0_16px_45px_rgba(26,50,99,0.06)]">
  ...
  return (
    <div className={cn('grid gap-5 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-[1.75rem] border border-[#1a32631f] bg-white/70 p-4 shadow-[0_16px_45px_rgba(26,50,99,0.06)]">
