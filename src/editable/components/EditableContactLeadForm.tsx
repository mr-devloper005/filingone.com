...
export function EditableContactLeadForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[2rem] border border-[#1a32631f] bg-white/90 p-6 shadow-[0_24px_70px_rgba(26,50,99,0.08)] backdrop-blur md:p-8">
      ...
      <label className="mt-4 grid gap-2 text-sm font-black opacity-75">
        Message
        <textarea name="message" required rows={6} placeholder="Tell us what you need help with..." className="rounded-2xl border border-[#1a32631f] bg-[var(--slot4-gray)] px-4 py-3 text-base font-medium outline-none transition focus:border-[var(--slot4-accent)]" />
      </label>
...
function Field({ name, label, type = 'text', placeholder, required = false }: { name: string; label: string; type?: string; placeholder?: string; required?: boolean }) {
  return (
    <label className="grid gap-2 text-sm font-black opacity-75">
      {label}
      <input name={name} type={type} required={required} placeholder={placeholder} className="h-12 rounded-2xl border border-[#1a32631f] bg-[var(--slot4-gray)] px-4 text-base font-medium outline-none transition focus:border-[var(--slot4-accent)]" />
    </label>
  )
}
