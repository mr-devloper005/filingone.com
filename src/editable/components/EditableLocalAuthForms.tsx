...
const inputClass = 'h-12 rounded-2xl border border-[#1a32631f] bg-[var(--slot4-gray)] px-4 text-base font-bold text-current outline-none transition placeholder:text-current/35 focus:border-[var(--slot4-accent)] focus:bg-white'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-2xl bg-[var(--slot4-dark-bg)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalSignupForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      <input className={`${inputClass} text-black`} placeholder="Full name" value={name} onChange={(event) => setName(event.target.value)} required />
      <input className={inputClass} type="email" placeholder="Email address" value={email} onChange={(event) => setEmail(event.target.value)} required />
      <input className={inputClass} type="password" placeholder="Password" value={password} onChange={(event) => setPassword(event.target.value)} required />
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
