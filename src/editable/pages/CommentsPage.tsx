  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2.4rem] border border-[#1a32631f] bg-white p-6 shadow-[0_24px_70px_rgba(26,50,99,0.08)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <button type="button" className="rounded-full border border-[#1a32631f] bg-[var(--slot4-gray)] px-4 py-2 text-sm font-black transition hover:-translate-y-0.5" onClick={refreshComments}>Refresh comments</button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-11 w-full rounded-2xl border border-[#1a32631f] bg-[var(--slot4-gray)] pl-9 pr-3 text-sm outline-none focus:border-[var(--slot4-accent)] focus:bg-white"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-2xl border border-[#1a32631f] bg-white p-5 shadow-[0_18px_50px_rgba(26,50,99,0.06)]">
          ...
          <section className="mt-8 rounded-2xl border border-dashed border-[#1a326325] bg-white/75 p-8 text-center">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#1a32631f] bg-white p-4 text-sm text-[var(--slot4-muted-text)]">
            <span>Page {currentPage} of {totalPages}</span>
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-[#1a32631f] px-4 py-2 font-black disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
              <button type="button" className="rounded-full border border-[#1a32631f] px-4 py-2 font-black disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
            </div>
          </div>
        ) : null}
      </main>
