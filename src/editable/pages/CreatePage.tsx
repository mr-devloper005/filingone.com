...
const fieldClass = 'rounded-2xl border border-[#1a32631f] bg-[var(--slot4-gray)] px-4 py-3 text-sm font-bold text-[var(--editable-page-text,#121a2a)] outline-none transition placeholder:text-current/35 focus:border-[var(--slot4-accent)] focus:bg-white'
...
export default function CreatePage() {
  ...
  if (!session) {
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[var(--editable-page-bg,#f7f2ea)] px-4 py-16 text-[var(--editable-page-text,#121a2a)] sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 rounded-[2.8rem] border border-[#1a32631f] bg-white p-7 shadow-[0_24px_70px_rgba(26,50,99,0.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link href="/signup" className="inline-flex items-center gap-2 rounded-full border border-[#1a32631f] bg-white px-6 py-3 text-sm font-black">Sign up</Link>
              </div>
            </div>
          </section>
        </main>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#f7f2ea)] text-[var(--editable-page-text,#121a2a)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[2.8rem] border border-[#1a32631f] bg-white p-6 shadow-[0_24px_70px_rgba(26,50,99,0.08)] backdrop-blur lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-[var(--slot4-dark-bg)] bg-[var(--slot4-dark-bg)] text-white' : 'border-[#1a32631f] bg-[var(--slot4-gray)] hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-[2.2rem] border border-[#1a32631f] bg-[var(--slot4-gray)] p-5 sm:p-7">
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[var(--slot4-dark-bg)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5">
